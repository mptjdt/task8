...
namespace 墨心8A1
{
    public class InfoPanel : MonoBehaviour
    {
        ...
        // 当接收到地块点击事件时，更新文本框内容
        private void UpdateInfo(TileInfo.TileType tileType, int count)
        {
            if (infoText != null && panel != null)
            {
                UpdateText(tileType, count); // 更新显示的地块信息
                ShowInfoPanel();
            }
        }

        // 更新文本内容
        private void UpdateText(TileInfo.TileType tileType, int count)
        {
            string tileName = tileType.ToString(); // 将枚举转换为字符串
            if (count == -1)
            {
                infoText.text = $"点击的地块: {tileName}\n数量: ∞"; // 显示为正无穷
            }
            else
            {
                infoText.text = $"点击的地块: {tileName}\n数量: {count}"; // 更新文本框内容
